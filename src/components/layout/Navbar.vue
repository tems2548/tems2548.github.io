    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16 items-center">
        <div class="flex items-center gap-3 font-mono-tech">
          <div class="flex-shrink-0 font-black text-xl text-white tracking-tighter cursor-pointer logo-glitch">
        ...
        <div class="hidden md:flex items-center space-x-1 font-mono-tech uppercase">
          <a v-for="(label, id, idx) in { home: 'Home', about: 'About', working: 'Dev', contact: 'Link' }" 
             :key="id" 
             :href="'#' + id" 
             class="relative px-4 py-2 text-[10px] tracking-widest transition-all duration-300 group"
    ...
    <Transition name="fade">
      <div v-if="isMenuOpen" class="md:hidden bg-slate-950 border-t border-indigo-500/20 font-mono-tech uppercase">
        <div class="px-4 pt-2 pb-6 space-y-4">
          <a v-for="(label, id, idx) in { home: 'Home', about: 'About', working: 'Dev', resources: 'Vault', contact: 'Link' }" 
             :key="id"
             :href="'#' + id" 
